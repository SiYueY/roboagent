...
from roboagent import Agent as TopLevelAgent
from roboagent import AgentSession as TopLevelAgentSession
...
from roboagent.runtime.types import content_summary
from roboagent.runtime import (
    FileSource,
    MediaLimits,
    MediaOwnership,
...
class _Model:
    ...
    async def stream(self, request, cancellation):
        ...
        if isinstance(reply, str):
            ...
            reply = AssistantMessage((TextContent(reply),))
        yield ModelCompleted(reply)


class KernelTests(unittest.IsolatedAsyncioTestCase):
    def test_top_level_api_exports_stable_agent_types(self):
        self.assertIs(TopLevelAgent, Agent)
        self.assertEqual(TopLevelAgentSession.__name__, "AgentSession")

    def test_content_summary_never_retains_media_payload(self):
        for content in (
            ImageContent(BytesSource(b"image-bytes"), "image/png"),
            AudioContent(BytesSource(b"audio-bytes"), "audio/wav"),
            FileContent(BytesSource(b"file-bytes"), "text/plain"),
        ):
            summary = content_summary(content)
            self.assertNotIn(content.source.data.decode(), repr(summary))
            self.assertEqual(summary.size, len(content.source.data))
    ...
    async def test_session_captures_media_limits_at_creation(self):
        ...
        original_model = _Model(("done",))
        original_model.capabilities = capabilities
        session = Agent(original_model, media_limits=original_limits).new_session()
        replacement_model = _Model(("done",))
        replacement_model.capabilities = capabilities
        session.agent = Agent(
            replacement_model, media_limits=MediaLimits(max_inline_bytes=1)
        )

        result = await session.run(
            UserMessage(
                (ImageContent(BytesSource(b"ok"), "image/png"),), limits=original_limits
            )
        )
    ...
    async def test_failed_eager_start_rolls_back_session_state(self):
        ...
        with patch(
            "roboagent.agent.run.AgentRun.start_eager",
    ...
    async def test_multi_tool_exchange_is_complete_and_ordered(self):
        ...
        result = await session.run(
            "go",
            config=RunConfig(
                tool_execution=ToolExecutionConfig(ToolExecutionMode.PARALLEL)
            ),
        )
        self.assertEqual(result.status.value, "completed")
        self.assertEqual(
    ...
    async def test_parallel_tool_events_follow_completion_but_transcript_follows_call_order(
        self,
    ):
        calls = (ToolCall("a", "tool", "{}", {}), ToolCall("b", "tool", "{}", {}))
        model = _Model((AssistantMessage((), calls), "done"))
        ...
        await run.result()
        events = [
            event async for event in run.events() if event.type == "tool_completed"
        ]
        transcript = [
            message.tool_call_id
            for message in session.messages
    ...
    async def test_jsonl_event_store_uses_redacted_event_codec(self):
        with TemporaryDirectory() as directory:
            ...
            event = AgentEvent(
                ...
                1,
                "model_completed",
                content=(ContentSummary(Modality.IMAGE, "image/png", "bytes", 5),),
            )
    ...
    async def test_invalid_incremental_tool_call_is_model_error_without_assistant_commit(
        self,
    ):
        ...
        class Failing(_Model):
            async def stream(self, request, cancellation):
                raise MediaResolutionError(
                    MediaResolutionErrorCode.FETCH_FAILED, "safe"
                )
    ...
    async def test_mixed_stream_preserves_non_text_content_order(self):
        ...
        self.assertEqual(
            [type(value) for value in result.final_message.content],
            [TextContent, ImageContent, TextContent],
        )
        self.assertEqual(
            [
                value.text
                for value in result.final_message.content
    ...
    async def test_openai_adapter_generates_missing_tool_call_id(self):
        ...
        items = [
            ...
            async for item in _stream_chunks(
                Chunks(), RuntimeCancellation(), "provider"
            )
        ]
        delta = next(
            item for item in items if isinstance(item, ToolCallDelta) and item.call_id
        )
    ...
    async def test_lifecycle_hooks_are_observational_and_fail_open(self):
        ...
        class Hooks:
            ...
            def on_turn_start(self, context):
                seen.append("turn_start")

            def on_model_start(self, context):
                seen.append("model_start")

            def on_model_end(self, message):
                seen.append("model_end")

            def on_tool_start(self, call):
                seen.append("tool_start")

            def on_tool_end(self, outcome):
                seen.append(("tool_end", outcome.status.value))

            def on_turn_end(self, context):
                seen.append("turn_end")
        ...
        with self.assertLogs("roboagent.agent.run", "WARNING") as logs:
            result = (
                await Agent(
                    model, tools=(Tool("tool", "", {}, handler),), hooks=Hooks()
                )
                .new_session()
                .run("go")
            )
    ...
    async def test_pending_follow_up_becomes_next_turn(self):
        class Delayed(_Model):
            async def stream(self, request, cancellation):
                ...
                yield ModelCompleted(
                    AssistantMessage((TextContent(next(self.replies)),))
                )
    ...
    async def test_continue_run_creates_new_execution_without_new_user_message(self):
        ...
        self.assertEqual(
            (first.status.value, second.status.value), ("completed", "completed")
        )
        self.assertEqual(
            [message.role for message in session.messages],
    ...
    async def test_cancelled_tool_batch_still_commits_terminal_results(self):
        ...
        self.assertIn(
            tools[1].status, {ToolCallStatus.CANCELLED, ToolCallStatus.SKIPPED}
        )
        TranscriptValidator(session.agent.media_limits).validate(session.messages)
    ...
    async def test_cancel_does_not_start_pending_bounded_parallel_tool(self):
        ...
        run = session.start(
            ...
            config=RunConfig(
                tool_execution=ToolExecutionConfig(
                    ToolExecutionMode.PARALLEL, max_concurrency=1
                )
            ),
        )
        await started.wait()
        ...
        self.assertEqual(
            [message.status for message in tools],
            [ToolCallStatus.CANCELLED, ToolCallStatus.SKIPPED],
        )
    ...
    async def test_steer_observes_tools_without_early_transcript_commit(self):
        ...
        self.assertEqual(
            [message.role for message in session.messages], ["user", "assistant"]
        )
        release.set()
        result = await asyncio.wait_for(run.result(), 1)
        self.assertEqual(result.status.value, "completed")
        ...
        self.assertEqual(
            [message.status for message in tools],
            [ToolCallStatus.COMPLETED, ToolCallStatus.SKIPPED],
        )
        self.assertEqual(
            [message.role for message in session.messages],
    ...
    async def test_cancel_commits_received_pending_control_at_final_boundary(self):
        ...
        self.assertIn(
            "cancellation_requested", [event.type async for event in run.events()]
        )
    ...
    async def test_timeout_preserves_timed_out_status(self):
        ...
        result = (
            await Agent(Slow(()))
            .new_session()
            .run("go", config=RunConfig(timeout=0.01))
        )
        self.assertEqual(result.status.value, "timed_out")
        self.assertEqual(result.turns, 1)

    async def test_max_turns_counts_model_invocations_and_preserves_complete_tool_exchange(
        self,
    ):
        calls = (ToolCall("a", "tool", "{}", {}),)
    ...
    async def test_tool_model_context_is_opt_in(self):
        ...
        model = _Model((AssistantMessage((), calls), "done"))
        await (
            Agent(model, tools=(Tool("tool", "", {}, handler),)).new_session().run("go")
        )
        ...
        await (
            Agent(
                model, tools=(Tool("tool", "", {}, handler, expose_model_context=True),)
            )
            .new_session()
            .run("go")
        )
    ...
    def test_canonical_media_and_transcript_validation(self):
        with self.assertRaises(ProtocolError):
            UserMessage("  ", limits=Agent(_Model(())).media_limits)
        user = UserMessage(
            (TextContent(""), ImageContent(BytesSource(b"x"), "image/png")),
            limits=Agent(_Model(())).media_limits,
        )
        self.assertEqual(len(user.content), 2)
        with self.assertRaises(ProtocolError):
            TranscriptValidator(Agent(_Model(())).media_limits).validate(
                (ToolResultMessage("x", "t", ToolCallStatus.COMPLETED),)
            )
        self.assertEqual(AssistantMessage("hello").content, (TextContent("hello"),))
        self.assertEqual(
            ToolResultMessage(
                "x", "t", ToolCallStatus.COMPLETED, [TextContent("ok")]
            ).content,
            (TextContent("ok"),),
        )
    ...
    async def test_external_media_uses_resolver_and_closes_resource(self):
        class Resolver:
            ...
            async def resolve(
                self, source, *, expected_media_type, run_context, cancellation
            ):
                self.request = (source, expected_media_type, run_context)
                self.resource = ResolvedMedia(
                    b"image", "image/png", 5, source, MediaOwnership.BORROWED
                )
        ...
        encoded, resources = await _content(
            (ImageContent(FileSource("/tmp/image.png"), "image/png"),), request
        )
    ...
    async def test_external_media_resolution_is_parallel_but_content_order_is_canonical(
        self,
    ):
        class Resolver:
            def __init__(self):
            ...
            async def resolve(
                ...
                if source.path.endswith("first.png"):
                    ...
                    payload = b"second"
                resource = ResolvedMedia(
                    payload, "image/png", len(payload), source, MediaOwnership.BORROWED
                )
        ...
        self.assertTrue(
            encoded[0]["image_url"]["url"].endswith(base64.b64encode(b"first").decode())
        )
        self.assertEqual(encoded[1], {"type": "text", "text": "between"})
        self.assertTrue(
            encoded[2]["image_url"]["url"].endswith(
    ...
    async def test_external_media_failure_cancels_sibling_and_closes_successes(self):
        ...
        class Resolver:
            ...
            async def resolve(self, source, **kwargs):
                ...
                self.resource = ResolvedMedia(
                    b"good", "image/png", 4, source, MediaOwnership.BORROWED
                )
                try:
                    await asyncio.sleep(30)
        ...
        request = ModelRequest(
            "model",
            ModelContext(None, (), ()),
            RunContext("session", "run", RuntimeCancellation()),
            resolver,
        )
    ...
    async def test_openai_adapter_encodes_canonical_tool_exchange(self):
        ...
        request = ModelRequest(
            "model", context, RunContext("session", "run", RuntimeCancellation())
        )
        ...
        self.assertEqual(
            messages[2]["tool_calls"][0]["function"]["arguments"], '{"q":"x"}'
        )
