...
def test_schema_validation_runs_before_policy() -> None:
    async def check() -> None:
        ...
        batch = await ToolExecutor(
            registry=ToolRegistry((tool,)), policy=Policy()
        ).execute(
            (
                ToolCall("a", "missing", FrozenJsonObject()),
                ToolCall("b", "work", FrozenJsonObject()),
            ),
            _context(),
        )
        assert seen == [("missing", True)]
        assert [result.error.code for result in batch.results if result.error] == [
            "rejected",
            "invalid_arguments",
        ]
...
def test_concurrent_batch_is_bounded_and_results_keep_call_order() -> None:
    async def check() -> None:
        ...
        calls = tuple(
            ToolCall(str(value), "work", FrozenJsonObject({"value": value}))
            for value in range(4)
        )
        batch = await ToolExecutor(
            registry=ToolRegistry((tool,)), config=ToolExecutorConfig(max_concurrency=2)
...
def test_fail_run_aborts_and_retry_safe_formula() -> None:
    async def check() -> None:
        ...
        with pytest.raises(ToolBatchAborted) as caught:
            await ToolExecutor(registry=ToolRegistry(), policy=Policy()).execute(
                (ToolCall("x", "missing"),), _context()
            )
...
def test_timeout_is_a_terminal_result_and_invalid_output_aborts() -> None:
    async def check() -> None:
        ...
        assert (
            batch.results[0].error is not None
            and batch.results[0].error.code == "timeout"
        )
        assert batch.effects[0].status is ToolEffectStatus.SUCCEEDED

        invalid = Tool(
            _definition("invalid"), lambda arguments, context: "legacy output"
        )
        with pytest.raises(ToolBatchAborted) as caught:
...
def test_side_effecting_generic_exception_is_unknown() -> None:
    async def check() -> None:
        ...
        read_batch = await ToolExecutor(registry=ToolRegistry((read_only,))).execute(
            (call,), _context()
        )
        ...
        side_effecting = Tool(
            _definition(), raises, effect_kind=ToolEffectKind.SIDE_EFFECTING
        )
        side_batch = await ToolExecutor(
            registry=ToolRegistry((side_effecting,))
    ...
    async def check() -> None:
        ...
        async def failed(arguments, context):
            try:
                ...
                raise ToolExecutionFailure(
                    ToolErrorInfo("not_applied", "Action was not applied.")
                ) from exc
        ...
        async def uncertain(arguments, context):
            try:
                ...
                raise ToolEffectUnknown(
                    ToolErrorInfo("ack_lost", "Action acknowledgement was lost.")
                ) from exc
        ...
        for effect_kind, handler, expected_status, expected_error in cases:
            ...
            batch = await ToolExecutor(
                ...
                (ToolCall("t", "work", FrozenJsonObject({"value": 1})),), _context()
            )
            assert (
                batch.results[0].error is not None
                and batch.results[0].error.code == "timeout"
            ...
            expected_retry_safe = not (
                ...
                and expected_status
                in {ToolEffectStatus.SUCCEEDED, ToolEffectStatus.UNKNOWN}
            )
...
def test_cancel_effect_status_is_conservative_for_side_effects() -> None:
    async def check(effect_kind: ToolEffectKind) -> tuple[ToolEffectStatus, str]:
        ...
        assert retry_safe(caught.value.effects) is (
            effect_kind is ToolEffectKind.READ_ONLY
        )
        return effect.status, effect.error.code

    assert asyncio.run(check(ToolEffectKind.READ_ONLY)) == (
        ToolEffectStatus.CANCELLED,
        "cancelled",
    )
    assert asyncio.run(check(ToolEffectKind.SIDE_EFFECTING)) == (
        ToolEffectStatus.UNKNOWN,
        "effect_unknown",
    )
...
def test_filesystem_tools_are_scoped_atomic_and_deterministic(tmp_path: Path) -> None:
    async def check() -> None:
        ...
        tools = {
            ...
            for tool in create_filesystem_tools(FilesystemConfig(workspace))
        }
        context = _context()
        written = await tools["write_file"].execute(
            FrozenJsonObject(
                {"path": "a/file.txt", "content": "héllo", "create_parents": True}
            ),
            context,
        )
        assert (
            isinstance(written, ToolJsonContent) and written.value["bytes_written"] == 6
        )
        read = await tools["read_file"].execute(
            FrozenJsonObject({"path": "a/file.txt", "offset": 1, "limit": 3}), context
        )
        assert isinstance(read, ToolTextContent) and read.text == "éll"
        await tools["write_file"].execute(
            FrozenJsonObject({"path": "b.txt", "content": "x"}), context
        )
        found = await tools["find_files"].execute(
            FrozenJsonObject({"pattern": "**/*.txt"}), context
        )
        assert isinstance(found, ToolJsonContent)
        assert [item["path"] for item in found.value["items"]] == [
            "a/file.txt",
            "b.txt",
        ]
        with pytest.raises(Exception):
            await tools["read_file"].execute(
                FrozenJsonObject({"path": "../outside"}), context
            )
...
def test_shell_is_noninteractive_scoped_and_bounded(tmp_path: Path) -> None:
    async def check() -> None:
        ...
        shell = create_shell_tool(
            ShellConfig(
                FilesystemWorkspace(tmp_path),
                max_stdout_bytes=3,
                env=FrozenJsonObject({"ROBOAGENT_TEST": "yes"}),
            )
        )
        result = await shell.execute(
            FrozenJsonObject(
                {
                    "command": "printf 12345; printf err >&2; printf $ROBOAGENT_TEST > marker",
                    "cwd": "sub",
                }
            ),
            _context(),
        ...
        assert (
            result.value["stdout"] == "123" and result.value["stdout_truncated"] is True
        )
        assert result.value["stderr"] == "err"
        assert (tmp_path / "sub" / "marker").read_text() == "yes"
        assert dict(os.environ) == before

        background = create_shell_tool(
            ShellConfig(FilesystemWorkspace(tmp_path), cancellation_grace_period=0.02)
        )
        ...
        assert (
            isinstance(completed, ToolJsonContent) and completed.value["exit_code"] == 0
        )
