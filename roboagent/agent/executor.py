...
from roboagent.message import TextContent, ToolCall, ToolCallStatus, ToolExecutionError, ToolResultMessage
from roboagent.runtime.types import CancellationReason, ContentSummary, ModelContext, Modality, RunContext, RuntimeCancellation, modality
...
class ToolExecutionPolicy:
    ...
    def on_steer(self, state: ToolCallState) -> SteeringAction:
        return SteeringAction.SKIP if state is ToolCallState.PENDING else SteeringAction.CONTINUE
...
class ToolExecutor:
    ...
    def _skipped(call: ToolCall, code: str = "batch_stopped") -> ToolCallOutcome:
        return ToolCallOutcome(call.id, call.name, ToolCallStatus.SKIPPED, error=ToolExecutionError(code, "Tool batch stopped."))

    @staticmethod
    def _cancelled(call: ToolCall) -> ToolCallOutcome:
        return ToolCallOutcome(call.id, call.name, ToolCallStatus.CANCELLED, error=ToolExecutionError("cancelled", "Tool execution cancelled."))

    async def execute(self, calls: tuple[ToolCall, ...], run_context: RunContext, model_context: ModelContext) -> ToolExecutionBatchResult:
        ...
        async def report_start(call: ToolCall) -> None:
            ...
            if self.emit is None:
                return
            value = self.emit("tool_started", turn=run_context.turn, tool_call_id=call.id, tool_name=call.name)
            if inspect.isawaitable(value):
                await value
        ...
        async def report_terminal(outcome: ToolCallOutcome) -> None:
            ...
            if outcome.output is not None:
                content = tuple(ContentSummary(Modality.TEXT, size=len(part.text)) if isinstance(part, TextContent) else ContentSummary(modality(part), getattr(part, "media_type", None), type(getattr(part, "source", None)).__name__.removesuffix("Source").lower(), len(part.source.data) if hasattr(getattr(part, "source", None), "data") else None) for part in outcome.output.content)
            event_type = {
            ...
            value = self.emit(event_type, turn=run_context.turn, tool_call_id=outcome.call_id, tool_name=outcome.tool_name, status=outcome.status.value, content=content, error_code=outcome.error.code if outcome.error else None)
            if inspect.isawaitable(value):
                await value
        ...
        async def one(call: ToolCall, child: RuntimeCancellation) -> ToolCallOutcome:
            try:
                ...
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, error=ToolExecutionError("policy_exception", "Tool policy failed before execution."))
            if action is BeforeToolAction.SKIP:
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.SKIPPED, error=ToolExecutionError("policy_skipped", "Tool skipped by policy."))
            if action in {BeforeToolAction.REJECT, BeforeToolAction.FAIL_RUN}:
                code = "policy_fail_run" if action is BeforeToolAction.FAIL_RUN else "policy_denied"
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, error=ToolExecutionError(code, "Tool rejected by policy."))
            if child.cancelled:
                return self._cancelled(call)
            tool = self.tools.get(call.name)
            if tool is None:
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, error=ToolExecutionError("unknown_tool", "Unknown tool."))
            if call.parse_error or call.arguments is None:
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, error=ToolExecutionError("invalid_arguments", "Invalid tool arguments."))
            params = tool.validate(call.arguments)
            if isinstance(params, ToolExecutionError):
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, error=params)
            try:
                await report_start(call)
                invocation_context = model_context if tool.expose_model_context else None
                task = asyncio.create_task(tool.invoke(params, ToolInvocation(call, run_context, ToolCallContext(call.id, child), invocation_context), limits=self.limits))
                ...
                output = task.result()
                if output.is_error:
                    return ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, output, ToolExecutionError(output.error_code or "tool_error", "Tool returned an error."))
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.COMPLETED, output)
            except asyncio.CancelledError:
                return self._cancelled(call)
            except InvalidToolOutputError:
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, error=ToolExecutionError("invalid_tool_output", "Tool returned invalid output."))
            except Exception:
                return ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, error=ToolExecutionError("execution_error", "Tool execution failed."))
        ...
        if self.config.mode is ToolExecutionMode.SEQUENTIAL:
            ...
            def new_steer_actions(state: ToolCallState) -> tuple[SteeringAction, ...]:
                ...
                for control in sorted(self.observe_controls(), key=lambda item: getattr(item, "sequence", 0)):
            ...
            async def execute_one(call: ToolCall) -> tuple[ToolCallOutcome, bool]:
                ...
                while not task.done() and self.wait_for_control is not None:
                    signal = asyncio.create_task(self.wait_for_control(observed_control))
                    ...
                    try:
                        ...
                        pending_actions = tuple(self.policy.on_steer(ToolCallState.PENDING) for _ in actions)
                    except Exception:
                    ...
                    if SteeringAction.CANCEL in actions:
                        child.cancel(CancellationReason.TOOL_POLICY)
                    if any(action in {SteeringAction.SKIP, SteeringAction.CANCEL} for action in pending_actions):
            ...
            for call in calls:
                if stop or run_context.cancellation.cancelled:
                    outcome = self._skipped(call, "cancelled_before_start" if run_context.cancellation.cancelled else "batch_stopped")
                    built.append(outcome); await report_terminal(outcome)
                    continue
                try:
                    pending_actions = new_steer_actions(ToolCallState.PENDING)
                except Exception:
                    outcome = ToolCallOutcome(call.id, call.name, ToolCallStatus.FAILED, error=ToolExecutionError("policy_exception", "Tool policy failed while handling steering."))
                    built.append(outcome); await report_terminal(outcome)
                    stop = fail_run = True
                    continue
                if skip_remaining_for_steer or any(action in {SteeringAction.SKIP, SteeringAction.CANCEL} for action in pending_actions):
        ...
        fail_run = False
        observed_control = 0
        running: dict[asyncio.Task[ToolCallOutcome], tuple[int, RuntimeCancellation]] = {}
            ...
            if run_context.cancellation.cancelled:
                while next_index < len(calls):
                    if built[next_index] is None:
                        built[next_index] = self._skipped(calls[next_index], "cancelled_before_start")
                    next_index += 1
                return
        ...
        def apply_steering() -> None:
            ...
            if self.observe_controls is None:
                return
            controls = sorted(self.observe_controls(), key=lambda control: getattr(control, "sequence", 0))
        ...
        while running:
            ...
            if self.wait_for_control is not None:
                control_wait = asyncio.create_task(self.wait_for_control(observed_control))
                wait_set.add(control_wait)  # type: ignore[arg-type]
            completed, _ = await asyncio.wait(wait_set, return_when=asyncio.FIRST_COMPLETED)
        ...
        for index in range(next_index, len(calls)):
            built[index] = self._skipped(calls[index], "cancelled_before_start" if run_context.cancellation.cancelled else "batch_stopped")
            await report_terminal(built[index])
        for outcome in built:
            if outcome is not None:
                await report_terminal(outcome)
        return ToolExecutionBatchResult(tuple(outcome for outcome in built if outcome is not None), fail_run)


def tool_result(outcome: ToolCallOutcome, *, limits: object = None) -> ToolResultMessage:
    options = {"limits": limits} if limits is not None else {}
    if outcome.status is ToolCallStatus.COMPLETED:
        assert outcome.output is not None
        return ToolResultMessage(outcome.call_id, outcome.tool_name, outcome.status, outcome.output.content, **options)
    message = outcome.error.message if outcome.error else outcome.status.value
    return ToolResultMessage(outcome.call_id, outcome.tool_name, outcome.status, (TextContent(message),), outcome.error, **options)
