...
class InvalidContinuationError(ProtocolError):
    pass
...
class AgentSession:
    ...
    _active: AgentRun | None = field(default=None, init=False, repr=False)
    ...
    def start(
        ...
        *,
        ...
        message = (
            ...
            else UserMessage(prompt, limits=self._media_limits)
        )
    ...
    def continue_run(
        ...
        *,
        ...
        if not self._messages:
            raise InvalidContinuationError("Cannot continue an empty Session.")
        try:
            TranscriptValidator(self._media_limits).validate(self._messages)
        except ProtocolError as exc:
            raise InvalidContinuationError(str(exc)) from exc
        from .run import AgentRun
    ...
    def _append(self, message: Message) -> None:
        TranscriptValidator(self._media_limits).validate(
            (*self._messages, message), complete=False
        )
        self._messages.append(message)
