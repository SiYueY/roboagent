...
class ToolOutput:
    ...
    def __init__(
        self,
        content: Iterable[MessageContent] | str = (),
        is_error: bool = False,
        error_code: str | None = None,
        details: Any = None,
        *,
        ...
        if error_code is not None:
            try:
                ...
                raise ValueError(
                    "ToolOutput.error_code must be a safe error identifier."
                ) from exc
...
class Tool:
    ...
    handler: Callable[[Mapping[str, Any], ToolInvocation], Any]
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "Tool name must be non-empty str and description must be str."
            )
    ...
    ) -> Mapping[str, Any] | ToolExecutionError:
    ...
    async def invoke(
        self, params: Mapping[str, Any], invocation: ToolInvocation, *, limits: object
    ) -> ToolOutput:
        ...
        try:
            ...
            if isinstance(value, str):
                return ToolOutput(value, limits=limits)
        except (TypeError, ValueError) as exc:
            raise InvalidToolOutputError(
                "Tool output is not canonical MessageContent."
            ) from exc
