...
from roboagent.runtime import CancellationToken, ExecutionLineage
...
class ApprovalRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.arguments_digest != canonical_json_digest(self.arguments):
            raise ValueError(
                "ApprovalRequest arguments_digest does not match arguments."
            )
        if self.reason is not None and not isinstance(self.reason, str):
            raise TypeError("ApprovalRequest.reason must be str or None.")
        if self.lineage is not None and not isinstance(self.lineage, ExecutionLineage):
            raise TypeError("ApprovalRequest.lineage must be ExecutionLineage or None.")
        if self.effect_capability not in {None, "read_only", "side_effecting"}:
            raise ValueError("ApprovalRequest.effect_capability is invalid.")
