...
def _require_sdk():
    try:
        ...
        raise RuntimeError("DashScope speech support requires `pip install roboagent[speech]`.") from exc
    ...
    async def start(self) -> None:
        ...
        class Callback(RecognitionCallback):
            ...
            def on_event(self, result: Any) -> None:
                ...
                text = _field(sentence, "text", "")
                if text:
                    final = RecognitionResult.is_sentence_end(sentence)
                    loop.call_soon_threadsafe(queue.put_nowait, Transcript(text=text, final=final))
            def on_error(self, message: Any) -> None:
                detail = _field(message, "message", None) or _field(message, "code", None) or repr(message)
        ...
        try:
            ...
            self._recognition = dashscope.audio.asr.Recognition(
                model=config.model, format="pcm", sample_rate=16_000, callback=Callback(),
                workspace=config.workspace_id, language_hints=list(config.language_hints),
