...
class EnergyVAD:
    ...
    confidence = 0.0
    level = 0.0
    def __init__(self, threshold: float = 0.02, *, noise_multiplier: float = 3.0,
                 calibration_frames: int = 25) -> None:
...
class SileroVAD:
    ...
    def __init__(self, *, confidence: float = 0.5, min_volume: float = 0.003,
                 start_ms: int = 160, stop_ms: int = 400,
                 model_path: str | None = None, required: bool = False) -> None:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self._session = ort.InferenceSession(model, providers=["CPUExecutionProvider"], sess_options=opts)
            self._np = np
            ...
            self._available = False
            if self.required:
                raise SpeechConfigurationError("Silero VAD is configured as required but unavailable.") from exc
            logging.getLogger(__name__).warning(
                "Silero VAD unavailable; using RNNoise speech confidence when available, otherwise energy gate: %s", exc
            )
    ...
    def _confidence(self, data: bytes) -> float:
        ...
        samples = self._np.frombuffer(data, dtype=self._np.int16).astype(self._np.float32) / 32768.0
        model_input = self._np.concatenate((self._context, samples[None, :]), axis=1)
        output, self._state = self._session.run(None, {"input": model_input, "state": self._state, "sr": self._np.array(16000, dtype="int64")})
        self._context = model_input[:, -64:]
        return float(output[0][0])

    def process(self, audio: AudioChunk) -> bool:
        if audio.format.sample_rate != 16_000 or audio.format.sample_width != 2 or audio.format.channels != 1:
            raise SpeechConfigurationError("Silero VAD requires 16 kHz mono PCM16 input.")
        ...
        if not self._available:
            ...
            self._advance(speech and self.level >= self.min_volume, len(audio.data) / 32)
        ...
        while len(self._buffer) >= 1024:  # 512 samples / 32 ms at 16 kHz
            ...
            speaking = self._advance(self.confidence >= self.threshold and self.level >= self.min_volume, 32)
    ...
    def _advance(self, raw_speaking: bool, duration_ms: float) -> bool:
        if raw_speaking:
            ...
            if self.state in {VADState.QUIET, VADState.STOPPING}:
                self.state, self._start_seen_ms = VADState.STARTING, self._start_seen_ms + duration_ms
            ...
            if self.state == VADState.STARTING:
                ...
                self.state, self._stop_seen_ms = VADState.STOPPING, self._stop_seen_ms + duration_ms
